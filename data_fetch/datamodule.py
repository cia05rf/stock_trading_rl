...
class OHLCVDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        hdf5_path = hdf5_path if hdf5_path is not None else config.HDF5_FILE
        sequence_length = sequence_length if sequence_length is not None else config.SEQUENCE_LENGTH
        batch_size = batch_size if batch_size is not None else config.DM_BATCH_SIZE
        ...
        split_by_date = split_by_date if split_by_date is not None else config.SPLIT_BY_DATE
        train_end_date = train_end_date if train_end_date is not None else config.TRAIN_END_DATE
        val_end_date = val_end_date if val_end_date is not None else config.VAL_END_DATE
        normalize = normalize if normalize is not None else config.NORMALIZE
        include_volume = include_volume if include_volume is not None else config.INCLUDE_VOLUME
        include_value = include_value if include_value is not None else config.INCLUDE_VALUE
        target_type = target_type if target_type is not None else config.TARGET_TYPE
        ...
        respect_ticker_boundaries = respect_ticker_boundaries if respect_ticker_boundaries is not None else config.RESPECT_TICKER_BOUNDARIES
        super().__init__()
        ...
        self.respect_ticker_boundaries = respect_ticker_boundaries
    ...
    def _compute_normalization_stats(self, f: h5py.File, indices: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
